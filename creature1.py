...
class Creature:
  ...
  #I'm starting to implement these (I wasn't using them completely before) by starting with just using them on the Flytrap class
  
  
  def takeAction(self,game, num):
    self.num = num
    action = self.actions[self.num-1]
    if len(action) -1:
      if action[0] == self.infectN:
        self.infectN(self.ifEnemy(game), game, self.actions[self.num-1][1])
        return
      else:
